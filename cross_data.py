...
class ZuCo_dataset(Dataset):
    def __init__(self, input_dataset_dicts, phase, tokenizer, eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], setting = 'unique_sent', is_add_CLS_token = False, test_data = None):
        self.inputs = []
        ...
        for input_dataset_dict in input_dataset_dicts:
            ...
            # elif setting == 'unique_subj':
                ...
                train_divider = int(0.8*total_num_sentence)
                dev_divider = train_divider + int(0.19*total_num_sentence)
                test_subj = setting
                if test_subj not in subjects:
                    ...
                    subjects.remove(test_subj)
                    if test_data is not None:
                        subjects.remove(test_data)
                        if phase == 'test':
                            print(f'[INFO]initializing a test set using {test_data} setting...')
                            for i in range(total_num_sentence):
                                input_sample = get_input_sample(input_dataset_dict[test_data][i],self.tokenizer,eeg_type,bands = bands, add_CLS_token = is_add_CLS_token)
                                if input_sample is not None:
                                    self.inputs.append(input_sample)
                                else:
                                    ValueError(f'no valid test sample for {test_data}, please check your setting!')
                    if phase == 'train':
                        for i in range(total_num_sentence):
                                input_sample = get_input_sample(input_dataset_dict[test_subj][i],self.tokenizer,eeg_type,bands = bands, add_CLS_token = is_add_CLS_token)
                                if input_sample is not None:
                                    self.inputs.append(input_sample)
                    if phase == 'dev':
                        for i in range(dev_divider, total_num_sentence):
